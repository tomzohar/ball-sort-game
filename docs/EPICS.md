...
## E14 — Gameplay depth: feel + guidance _(shipped — E14.1–E14.8 complete)_
...
**Shipped:** E14.1 = B1 (capacity grows 4→6, gated behind the 1-empty-tube band; min-moves ceiling 24→48; curve rises through ~L20 to genuine expert — memory m21). E14.2 = B2 (skippable/replayable 3-step tutorial; `@AppStorage("hasSeenTutorial")` gate; "How to Play" in Settings). E14.3 = A1 (flying-ball pour-arc: pure unit-tested `PourGeometry`, `PourArcEffect` GeometryEffect for the true parabola, `BoardViewModel.lastMove` seam, landed ball suppressed in-flight; verified on Tom's device). E14.4 = A2 (drag-to-pour: a board-level `DragGesture` hit-tests source/dest off the captured `tubeRects`; lifts on grab via `tap`, pours on release via a new self-contained `BoardViewModel.pour(from:to:)` that shares tap's legality/feedback/pour-arc seam through an extracted private `commitMove`; coexists with taps via `minimumDistance`; verified on Tom's device). E14.8 = B4 (difficulty label — **already delivered by E12.7's `DifficultyBadgeView`**, which surfaces `Level N · Band` persistently; a separate level-intro announcement was considered and deferred as not worth its own overlay). E14.5 = A3 (win-moment bloom: `WinParticleBurst` — a calm 22-dot river-stone bloom behind the win card that drifts out and fades once; deterministic + snapshot-stable; honours Reduce Motion by rendering nothing). E14.6 = A4 (tube-completion settle: the final ball "locks" with a scale overshoot + a moss `CompletionRipple`, fired once per completion via a `settleToken` rising edge in `TubeView`; iOS-17 `keyframeAnimator`; skipped under Reduce Motion; inert at rest so snapshots are unchanged). E14.7 = B3 (smarter hints: a new `GameEvent.hint` wired through both feedback players — a gentle ascending sound + a soft haptic — fired from `requestHint` the moment a solver move surfaces, silent when none/won; plus an animated nudge where the hinted source+target tubes breathe via a slow `hintPulse`, Reduce-Motion-gated and inert at rest). **E14 complete — gameplay is feature-complete; the path to launch is now the E11 release pipeline (gated on Apple Developer approval).**
